...
function downloadInvoice(id) {
	var jobid = Query.select("Jobs.jobs", "*", "projectid =" + esc(id));
	for(i=0;i<jobid.length;i++) 	var duedate[i]=Query.select("Jobs.jobproducts","*", "jobid="+jobid[i].id);
	var date=friendlyDate(new Date());
	var clientinfo=Query.select("Contacts.Companies","*","name='Matric Drilling Pty Ltd' OR name='Matrix Drilling'");
		productid = Query.select("Jobs.jobproducts", "*", "jobid=" + esc(jobid[jobid.length].id));
		for (p = 0; p < productid.length; p++) {
			totalprice[p] = 0;
			if (productid[p].productid == null || productid[p].productid == "" || productid[p].productid == 0) {
				qty[c] = 0;
			} else {
				qty = Query.select("Jobs.jobproducts", "*", "productid=" + esc(productid[p].productid)); 
				for (q = 0; q < qtyjobproduct.length; q++) {
					if (qty[q].quantity == null || qty[q].quantity == "" || isNaN(qty[q].quantity) == true) {
						qty[q].quantity = 0;
					}
							
					totalprice[p] = totalprice[p] + (qty[q].quantity * productid[p].price);				
				}
			}
		}
	Pdf2.init();
	...
	for (c = 0; c < jobidduedate.length; c++) {
		...
		for (p = 0; p < productidjobproduct.length; p++) {
			...
			if (productidjobproduct[p].productid == null || productidjobproduct[p].productid == "" || productidjobproduct[p].productid == 0) {
				...
				qtyjobproduct = Query.select("Jobs.jobproducts", "*", "productid=" + esc(productidjobproduct[p].productid)); //+"AND duedate="+esc(jobidduedate[c].duedate));
				for (q = 0; q < qtyjobproduct.length; q++) {
					if (qtyjobproduct[q].quantity == null || qtyjobproduct[q].quantity == "" || isNaN(qtyjobproduct[q].quantity) == true) {
						qtyjobproduct[q].quantity = 0;
					}
					console.log(qtyjobproduct[q]);
					var s=q+"."+parseFloat(c);
					data1.push(s);
					map.set(data1[q],qtyjobproduct[q].quantity);
					//data1[q][c] = qtyjobproduct[q].quantity;
								...
								var products = Query.select("Sales.products", "*", "categoryid =" + esc(cat[i].id), "code")
									for (j = 0; j < products.length; j++) {
										if (products[j] == "" || products[j] == null) {}
											...
											data = [products[j].code, cat[i].name, products[j].name, products[j].price, Query.names("Sales.units", products[j].unitid), qtyjobproduct[q].quantity, totalprice[p]];
											for(c=0c<jobidduedate.length;c++){
											s=j+"."+parseFloat(c);
											data.push(map.get(s));
											}
											//data1.push(totalprice[p]);
											data.push(totalprice[j]);
											console.log(data);
										console.log(data);
									}
									console.log(data);
